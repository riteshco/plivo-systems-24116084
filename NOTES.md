...
6. **delay_ms**: We should grade at **`delay_ms 50`** for Profile A and **`delay_ms 150`** for Profile B. Profile A's 50ms score pushes exactly up against the boundary where network jitter physically delays recovery packets past the deadline. Profile B's heavy stress creates high network latencies and burst drops that physically require 150ms for consecutive NACK retransmissions to successfully arrive and reliably hold the <=1.00% miss rate cap.
